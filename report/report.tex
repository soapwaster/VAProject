...
During the development of this application, many other views have been taken into consideration, but at the end they were dropped because of the lack of significant insights provided by the data.\\
...
This view, instead, was a variation of the later used bar chart. Infact, it initially started as a \texttt{stacked bar chart}, where one layer was supposed to show the number of flights that departed from that state, in that particular month, while the other had the purpose to show the arrived flights. We immediately noticed that they were almost identical, probably because every flight has also a return one.\\ \\
Lastly, we dropped the idea of implementing a kind of \texttt{correlation matrix} (or a \texttt{chord diagram}) to show for each pair of states, the number of departed flights, in order to understand what are the main routes. This still is a nice idea, however it was not feasible due to the lack of space. A $50\times50$ matrix is too big to be shown on a screen that has other 4 views. A chord diagram on the other hand would look too caothic It may be interesting to be shown independently.
